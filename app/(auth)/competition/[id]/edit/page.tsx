...
export default function EditCompetitionPage() {
  ...
  // Load Categories & Competition Data
  useEffect(() => {
    async function loadData() {
      try {
        setLoading(true);
        const [cats, data] = await Promise.all([
          competitionService.getCompetitionCategories(),
          competitionService.getCompetitionById(compId)
        ]);

        // Guard access: check competition.update permission
        if (profile && !hasPermission("competition.update", compId, data.creator_id)) {
        ...
        // Map rules array to concatenated HTML string of paragraphs
        const rawRules = data.rules 
          ? data.rules.map(r => r.startsWith("<") ? r : `<p>${r}</p>`).join("") 
          : "";
        setRules(rawRules);

        // Prepopulate dates
        setRegStart(data.registration_start_date ? data.registration_start_date.split("T")[0] : "");
        setRegEnd(data.registration_end_date ? data.registration_end_date.split("T")[0] : "");
        setQualStart(data.qualification_start_date ? data.qualification_start_date.split("T")[0] : "");
        setQualEnd(data.qualification_end_date ? data.qualification_end_date.split("T")[0] : "");
        setFinalStart(data.final_start_date ? data.final_start_date.split("T")[0] : "");
        setFinalEnd(data.final_end_date ? data.final_end_date.split("T")[0] : "");

        // Prepopulate categories list
        if (data.category) {
          setSelectedCategories(data.category.split(",").map(c => c.trim()).filter(Boolean));
        }
        ...
        setErrorMsg("Gagal memuat detail kompetisi. Kompetisi tidak ditemukan.");
      } finally {
        setLoading(false);
      }
    }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      let posterUrl = posterPreview; // Keep old URL by default
      ...
      await competitionService.updateCompetition(compId, payload);

      router.push("/competition");
    } catch (err: any) {
      console.error("Save failed:", err);
      setErrorMsg(err.message || "Gagal menyimpan perubahan kompetisi.");
    } finally {
      setIsSaving(false);
    }
  };
  ...
  if (loading) {
    return (
      <div className="flex h-64 items-center justify-center bg-rose-650 text-zinc-100">
        <div className="flex flex-col items-center gap-3">
          <div className="h-10 w-10 animate-spin rounded-full border-4 border-slate-800 border-t-primary"></div>
          <p className="text-sm font-medium tracking-wide text-slate-400">Memuat detail kompetisi...</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-border pb-5">
        <div className="space-y-1">
          <nav className="flex items-center gap-1.5 text-xs text-slate-400">
            ...
            <span className="text-white font-medium">Edit Kompetisi</span>
          </nav>
          <h1 className="text-2xl font-bold tracking-tight text-white">Edit Kompetisi</h1>
        </div>

        <Button
          type="submit"
          form="edit-competition-form"
          disabled={isSaving}
      ...
      {/* Form Content */}
      <form id="edit-competition-form" onSubmit={handleSubmit} className="rounded-xl border border-border bg-card/10 p-4 sm:p-6 space-y-6">
        {/* Title */}
